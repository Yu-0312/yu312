...
function setupHomeExtras(repos) {
  ...
  if (likeBtn && likeCount) {
    const KEY = "max-neko-likes";
    let n = Number(localStorage.getItem(KEY) || "1");
    let on = sessionStorage.getItem("max-neko-liked") === "1";
    ...
    likeBtn.addEventListener("click", () => {
      if (sessionStorage.getItem("max-neko-liked") === "1") return;
      n += 1;
      localStorage.setItem(KEY, String(n));
      sessionStorage.setItem("max-neko-liked", "1");
      likeCount.textContent = String(n);
      likeBtn.classList.add("is-on");
      likeBtn.setAttribute("aria-pressed", "true");
    });
